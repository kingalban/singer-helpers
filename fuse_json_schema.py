...
def fuse_json_schemas(
    schema_a: JsonSchema | None = None, schema_b: JsonSchema | None = None
) -> JsonSchema:
    ...
    fused_schema: JsonSchema = {
        "type": union_types(schema_a.get("type", []), schema_b.get("type", []))
    }
    ...
    for key in all_keys:
        ...
        if key == "properties":
            ...
            for prop_key, val_a, val_b in zip_dicts(
                schema_a.get("properties", {}), schema_b.get("properties", {}), {}
            ):
                fused_schema["properties"][prop_key] = fuse_json_schemas(val_a, val_b)

        elif key == "items":
            fused_schema["items"] = fuse_json_schemas(
                *(sch["items"] for sch in source_schemas)
            )

        elif key == "format":
            if len(source_schemas) > 1:
                assert schema_a["format"] == schema_b["format"], (
                    f"irreconcilable formats: "
