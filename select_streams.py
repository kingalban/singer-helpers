...
def detect_unknown_streams(
    stream_names: Sequence[str], input_names: Sequence[str]
) -> None:
    unknown_names = set(input_names) - set(stream_names)
    if unknown_names:
        raise ValueError(
            "Warning: Streams you have referenced do not exist in the catalog!\n"
            "unknown streams: " + ", ".join(unknown_names)
        )
    return None
    ...
    for stream in catalog["streams"]:
        if replication_method:
            stream["replication-method"] = (
                replication_method  # some taps use this location in the document
            )
        ...
        for mdata in stream["metadata"]:
            if not mdata.get("breadcrumb"):
                ...
                if forced_replication_method:
                    mdata["metadata"]["forced-replication-method"] = (
...
def print_stream_summary(catalog: dict):
    print(
        f"| {'stream':<30} | {'selected':^10} | {'selected-by-default':^20} | {'replication-method':^20} |"
    )
    print(f"+-{'':-^30}-+-{'':-^10}-+-{'':-^20}-+-{'':-^20}-+")

    for stream in catalog["streams"]:
        metadata = [m for m in stream["metadata"] if m.get("breadcrumb") == []][0][
            "metadata"
        ]
        print(
            f"| {stream['stream']:<30} | "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        "select and de-select streams from singer catalog",
    ...
    parser.add_argument(
        "--mode", default="file", choices=["file", "print"], help="how to return result"
    )
    ...
    parser.add_argument(
        "--remove-schema",
        action="store_true",
        help="leave only '{}' schema for all streams",
    )
    parser.add_argument(
        "--replication-method",
        choices=["INCREMENTAL", "FULL_TABLE", "LOG_BASED"],
    ...
    filter_catalog(
        ...
        select,
        args.exclude,
        args.replication_method,
        args.forced_replication_method,
    )
